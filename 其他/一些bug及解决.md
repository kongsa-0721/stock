...
const phone = "15246795875";
  //substring
console.log(phone.substring(0, 3) + "****" + phone.substring(7));
  //正则表达式
console.log(phone.replace(/^(\d{3})\d{4}(\d+)/, "$1****$2"));
 ```
...
- 大布局 下面的div覆盖上面的div但是不影响上面 类似于抽屉
...
4、可以使用flex布局 使用 justify-content:space-between *顶级理解
...
添加属性  给div添加 tabIndex属性 attrs暂时还不会类型约束 ******
...
| git push **origin** kongsa:kongsasa (--force)强推            | 把本地分支推送到远程kongsasa上去 第二次就不用 -u             |
...
| git checkout -b kongsasa                                     | 在本地创建一个分支kongsasa 并且切换过去                      |
...
| git config --global http.sslVerify false           | 关闭https ssh                                    |
| git log --pretty=format:"%an %ai %s "              | 在一行打印日志                                   |
| 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           | to Img **token** in github in(备忘录) 这个废弃掉 |
...
| git checkout -b iss01                              | 新建一个iss01分支并切换过去                      |
...
- 查看安装的node版本 zsh下面可能不能查看nvm 由于过度配置了bash 暂时不要修改
